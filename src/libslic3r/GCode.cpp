                ...
                if (instance_to_print.object_by_extruder.support != nullptr) {
                    ...
                    if (is_overridden == (print_wipe_extrusions != 0))
                        gcode += this->extrude_support(
                            ...
                            instance_to_print.object_by_extruder.support->chained_path_from(m_last_pos.to_point(), support_extrusion_role));
...
ExtrusionPaths GCode::split_and_mapping_speed(double &other_path_v, double &final_v, ExtrusionPath &this_path, double max_smooth_length, bool split_from_left)
{
    ...
    Polyline3 input_polyline = this_path.polyline;
    if (!split_from_left)
...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed, bool use_seperate_speed, bool is_first_slope)
{
    ...
    // double z_diff = 0;
    // const ExtrusionPathContoured *contoured = dynamic_cast<const ExtrusionPathContoured *>(&path);
    //printf("CONTOURED %p %s %p\n", contoured, typeid(path).name(), &path);
    // if (contoured != nullptr) {
    //     z_diff = contoured->z_diffs.front();
    //     description += " contoured";
    // } else {
    //     description += " noncontoured";
    // }
    // bool z_diff_changed = std::abs(m_last_z_diff - z_diff) > EPSILON;
